...
export const MainLayout: React.FC<MainLayoutProps> = ({
    ...
    return (
        <div className="relative w-screen h-screen bg-black overflow-hidden flex flex-col md:flex-row">
            ...
            <div className={`absolute inset-0 pointer-events-none z-15 ${showMobileContent ? 'hidden md:block' : 'block'}`}>
            ...
            <div className={`w-full md:w-3/4 h-full relative z-20 flex items-center p-4 md:pl-4 md:pr-20 transition-all duration-700 ${activeSection === 'about' ? 'md:translate-x-12' : 'translate-x-0'} ${showMobileContent ? 'flex' : 'hidden md:flex'}`}>
                ...
                <motion.button
                    ...
                    className="md:hidden absolute top-4 left-4 z-50 bg-white text-black p-3 rounded-lg flex items-center gap-2 font-bold hover:bg-gray-200 transition-colors"
                    whileTap={{ scale: 0.95 }}
                >
                    <ArrowLeft size={20} />
                    MENU
                ...
                <div className={`relative transition-all duration-700 ease-in-out ${activeSection === 'about' ? 'w-full md:w-5/6 h-full md:h-4/5' : 'w-full md:w-4/5 h-full md:h-2/3'}`}>
                    ...
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-md border-2 border-white -skew-x-12 transform origin-bottom-left overflow-hidden">
                        <div className="skew-x-12 w-full h-full overflow-y-auto p-8 pt-16 md:pt-8 custom-scrollbar">
                            {children}
                        </div>
                    </div>
